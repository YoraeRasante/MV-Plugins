 ...
 * 
 * @param Show Window
 * @desc Show a window behind gauge
 * true false
 * @default false
 *
 * @help
...
YR.Param.PLGVShowWindow = (YR.Parameters['Show Window'].toLowerCase() === 'true');
...
Window_Base.prototype.drawPartyLimitSet = function(unit, x, y, w, h) {
    ...
    var height = Math.min(Window_Base._iconHeight * 2, h);
        this.drawPartyLimitIcon(this._unit, x + this.standardPadding(), y + this.standardPadding(), w - this.standardPadding() * 4, height - this.standardPadding());
        this.drawPartyLimitValue(this._unit, x + this.standardPadding(), y + this.standardPadding(), w - this.standardPadding() * 4, height - this.standardPadding());
        this.drawPartyLimitGauge(this._unit, x + this.standardPadding(), h - this.lineHeight() - this.standardPadding() * 2, w - this.standardPadding() * 2);
	} else {
	    var width = Math.min(Window_Base._iconWidth * 2, w);
    	this.drawPartyLimitIcon(this._unit, x, y, width - this.standardPadding(), h - this.standardPadding() * 2);
    	this.drawPartyLimitValue(this._unit, x, y, width - this.standardPadding(), h - this.standardPadding() * 2);
    	this.drawPartyLimitGauge(this._unit, x + this.standardPadding(), h - this.standardPadding(), w - this.standardPadding() * 2);
	}
};
...
Window_Base.prototype.drawPartyLimitIcon = function(unit, x, y, w, h) {
	if (YR.PLGV.GaugeOrientation() === 'horizontal') {
    	...
    	if (unit.isTroop()) {
    		x += this.width - Window_Base._iconWidth * 2 - this.standardPadding();
    	}
...
Window_Base.prototype.drawPartyLimitGauge = function(unit, x, y, w) {
	if (YR.PLGV.GaugeOrientation() === 'horizontal') {
        ...
        var gw = Math.floor((this.width - this.standardPadding() * 4) / gauges);
        if (gw >= 5) {
        for (var i = 0; i < gauges; ++i) {
            ...
            this.drawGauge(x, y, w - this.standardPadding() * 4, rate, c1, c2);
        }
	} else {
    	var gauges = unit.partyLimitGaugeIncrements();
    	var rates = unit.partyLimitGaugeLastRates();
    	...
    	var gh = Math.floor((this.height - this.standardPadding() * 2) / gauges);
    	var gx = x - this.standardPadding();
    	if (unit.isParty()) {
    		gx += this.width - gw - this.standardPadding() * 2;
    	}
    	if (gh >= 5) {
    	  for (var i = 0; i < gauges; ++i) {
    	    var rate = rates[i] || 0;
    	    this.drawVerticalGauge(gx, y - gh - this.standardPadding(), gw, gh, rate, c1, c2);
    	    y -= gh;
    	  }
    	} else {
    	  var rate = unit.partyLimitGaugeRate();
    	  this.drawVerticalGauge(gx, y, gw, w - this.standardPadding() * 4, rate, c1, c2);
    	}
	}
};
...
YR.PLGV.PLWinitialize = Window_PartyLimitGauge.prototype.initialize;
Window_PartyLimitGauge.prototype.initialize = function(unit) {
    YR.PLGV.PLWinitialize.call(this, unit);
    console.log(YR.Param.PLGVShowWindow);
    if (YR.Param.PLGVShowWindow) {
...
Window_PartyLimitGauge.prototype.standardPadding = function() {
    return YR.Param.PLGVShowWindow ? Window_Base.prototype.standardPadding() / 2 : 0;
};
